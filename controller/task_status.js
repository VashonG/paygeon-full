...
const finders = require('./../metadata/task_status.json')
...
const task_statusCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("task_status").aggregate(Agg(finders, filter, 'count')).toArray()
...
const task_statusList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("task_status").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const task_statusStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("task_status").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("task_status").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("task_status").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("task_status").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const task_statusDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("task_status").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/task_status.json")
    if (!collectionData) {
...
const findOnetask_statusItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("task_status");
  } else {
    dbCollection = await dbConnection.collection("task_status");
  }
...
const findtask_statusItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  ...
  findOnetask_statusItemByQuery,
  findtask_statusItemById,
