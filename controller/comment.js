...
const finders = require('./../metadata/comment.json')
...
const commentCount = async (req, res) => {
    try {
        ...
        const item = await db.collection("comment").aggregate(Agg(finders, filter, 'count')).toArray()
...
const commentList = async (req, res) => {
    try {
        ...
        const item = await db.collection("comment").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const commentStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("comment").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("comment").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("comment").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("comment").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const commentDetail = async (req, res) => {
    try {
        ...
        const _result = await db.collection("comment").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/comment.json")
    if (!collectionData) {
...
const findOnecommentItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("comment");
  } else {
    dbCollection = await dbConnection.collection("comment");
  }
