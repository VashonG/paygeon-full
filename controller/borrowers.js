...
const finders = require('./../metadata/borrowers.json')
...
const borrowersCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("borrowers").aggregate(Agg(finders, filter, 'count')).toArray()
...
const borrowersList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("borrowers").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const borrowersStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("borrowers").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("borrowers").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("borrowers").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("borrowers").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const borrowersDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("borrowers").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/borrowers.json")
    if (!collectionData) {
...
const findOneborrowersItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("borrowers");
  } else {
    dbCollection = await dbConnection.collection("borrowers");
  }
...
const findborrowersItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  ...
  findOneborrowersItemByQuery,
  findborrowersItemById,
