...
const finders = require('./../metadata/news_update.json')
...
const news_updateCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("news_update").aggregate(Agg(finders, filter, 'count')).toArray()
...
const news_updateList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("news_update").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const news_updateStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("news_update").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("news_update").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("news_update").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("news_update").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const news_updateDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("news_update").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/news_update.json")
    if (!collectionData) {
...
const findOnenews_updateItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("news_update");
  } else {
    dbCollection = await dbConnection.collection("news_update");
  }
...
const findnews_updateItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  ...
  findOnenews_updateItemByQuery,
  findnews_updateItemById,
