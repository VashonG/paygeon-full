...
const finders = require('./../metadata/team.json')
...
const teamCount = async (req, res) => {
    try {
        ...
        const item = await db.collection("team").aggregate(Agg(finders, filter, 'count')).toArray()
...
const teamList = async (req, res) => {
    try {
        ...
        const item = await db.collection("team").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const teamStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("team").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("team").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("team").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("team").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const teamDetail = async (req, res) => {
    try {
        ...
        const _result = await db.collection("team").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/team.json")
    if (!collectionData) {
...
const findOneteamItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("team");
  } else {
    dbCollection = await dbConnection.collection("team");
  }
