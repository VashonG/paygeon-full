...
const finders = require('./../metadata/redeem_miles.json')
...
const uniqueFields = ["alaska_airlines_mileage_number"]
const passwordFields = []

const redeem_milesCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("redeem_miles").aggregate(Agg(finders, filter, 'count')).toArray()
...
const redeem_milesList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("redeem_miles").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const redeem_milesStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("redeem_miles").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("redeem_miles").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("redeem_miles").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("redeem_miles").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const redeem_milesDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("redeem_miles").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/redeem_miles.json")
    if (!collectionData) {
...
const findOneredeem_milesItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("redeem_miles");
  } else {
    dbCollection = await dbConnection.collection("redeem_miles");
  }
...
const findredeem_milesItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  ...
  findOneredeem_milesItemByQuery,
  findredeem_milesItemById,
