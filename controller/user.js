...
const finders = require('./../metadata/user.json')
...
const userCount = async (req, res) => {
    try {
        ...
        const item = await db.collection("user").aggregate(Agg(finders, filter, 'count')).toArray()
...
const userList = async (req, res) => {
    try {
        ...
        const item = await db.collection("user").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const userStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("user").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("user").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("user").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("user").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const userDetail = async (req, res) => {
    try {
        ...
        const _result = await db.collection("user").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/user.json")
    if (!collectionData) {
...
const findOneuserItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("user");
  } else {
    dbCollection = await dbConnection.collection("user");
  }
