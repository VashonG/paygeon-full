...
const finders = require('./../metadata/assets_and_liabilities.json')
...
const assets_and_liabilitiesCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("assets_and_liabilities").aggregate(Agg(finders, filter, 'count')).toArray()
...
const assets_and_liabilitiesList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("assets_and_liabilities").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const assets_and_liabilitiesStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("assets_and_liabilities").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("assets_and_liabilities").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("assets_and_liabilities").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("assets_and_liabilities").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const assets_and_liabilitiesDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("assets_and_liabilities").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/assets_and_liabilities.json")
    if (!collectionData) {
...
const findOneassets_and_liabilitiesItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("assets_and_liabilities");
  } else {
    dbCollection = await dbConnection.collection("assets_and_liabilities");
  }
...
const findassets_and_liabilitiesItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  assets_and_liabilitiesDetail,
  editItem,
  deleteItem,
  assets_and_liabilitiesStoreData,
  assets_and_liabilitiesList,
  assets_and_liabilitiesCount,
  ...
  findOneassets_and_liabilitiesItemByQuery,
  findassets_and_liabilitiesItemById,
