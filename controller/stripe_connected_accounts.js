...
const finders = require('./../metadata/stripe_connected_accounts.json')
...
const uniqueFields = ["name","accountId"]
const passwordFields = []

const stripe_connected_accountsCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("stripe_connected_accounts").aggregate(Agg(finders, filter, 'count')).toArray()
...
const stripe_connected_accountsList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("stripe_connected_accounts").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const stripe_connected_accountsStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("stripe_connected_accounts").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("stripe_connected_accounts").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("stripe_connected_accounts").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("stripe_connected_accounts").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const stripe_connected_accountsDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("stripe_connected_accounts").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/stripe_connected_accounts.json")
    if (!collectionData) {
...
const findOnestripe_connected_accountsItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("stripe_connected_accounts");
  } else {
    dbCollection = await dbConnection.collection("stripe_connected_accounts");
  }
...
const findstripe_connected_accountsItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  stripe_connected_accountsDetail,
  editItem,
  deleteItem,
  stripe_connected_accountsStoreData,
  stripe_connected_accountsList,
  stripe_connected_accountsCount,
  ...
  findOnestripe_connected_accountsItemByQuery,
  findstripe_connected_accountsItemById,
