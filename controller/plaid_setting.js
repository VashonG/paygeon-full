...
const finders = require('./../metadata/plaid_setting.json')
...
const uniqueFields = []
const passwordFields = ["access_token"]

const plaid_settingCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("plaid_setting").aggregate(Agg(finders, filter, 'count')).toArray()
...
const plaid_settingList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("plaid_setting").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const plaid_settingStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("plaid_setting").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("plaid_setting").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("plaid_setting").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("plaid_setting").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const plaid_settingDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("plaid_setting").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/plaid_setting.json")
    if (!collectionData) {
...
const findOneplaid_settingItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("plaid_setting");
  } else {
    dbCollection = await dbConnection.collection("plaid_setting");
  }
...
const findplaid_settingItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  ...
  findOneplaid_settingItemByQuery,
  findplaid_settingItemById,
