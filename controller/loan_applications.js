...
const finders = require('./../metadata/loan_applications.json')
...
const uniqueFields = ["loan_application_number"]
const passwordFields = []

const loan_applicationsCount = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("loan_applications").aggregate(Agg(finders, filter, 'count')).toArray()
...
const loan_applicationsList = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        
        const item = await db.collection("loan_applications").aggregate(Agg(finders, filter, 'list', req.query)).toArray()
...
const loan_applicationsStoreData = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req
        const data = req.body
        ...
        const _result = await db.collection("loan_applications").insertOne(_res)
...
const deleteItem = async (req, res) => {
    try {
        ...
        const itemExist = await  db.collection("loan_applications").findOne({ uuid: filter })

        if(itemExist){
            await db.collection("loan_applications").updateOne({ uuid: filter }, { $set: { isDeleted: true }})
...
const editItem = async (req, res) => {
    try {
        ...
        const _result = await db.collection("loan_applications").updateOne(query, value)
        return res.status(200).json(_result)
    } catch (e) {
        console.log(e)
        return res.status(500).json({
...
const loan_applicationsDetail = async (req, res) => {
    try {
        const { filter } = req.params
        const { db } = req

        const _result = await db.collection("loan_applications").findOne({ uuid: filter })
...
const saveUserWithProvider = async (
    ...
    const collectionData = require("../metadata/loan_applications.json")
    if (!collectionData) {
...
const findOneloan_applicationsItemByQuery = async (
  dbConnection,
  projectRemoteDb,
  query
) => {
  ...
  if (projectRemoteDb) {
    dbCollection = await projectRemoteDb.collection("loan_applications");
  } else {
    dbCollection = await dbConnection.collection("loan_applications");
  }
...
const findloan_applicationsItemById = async (
  dbConnection,
  projectRemoteDb,
  itemId,
  initialQuery
...
module.exports = {
  loan_applicationsDetail,
  editItem,
  deleteItem,
  loan_applicationsStoreData,
  loan_applicationsList,
  loan_applicationsCount,
  ...
  findOneloan_applicationsItemByQuery,
  findloan_applicationsItemById,
